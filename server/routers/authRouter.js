...
import { isBlocked, recordLoginAttempt, resetAttempts } from "../utils/loginAttempts.js";
...
router.post('/login', async (req, res) => {
    ...
    if (isBlocked(ip)) {
        return res.status(429).send({ errorMessage: "You have used 3 attempts. Try again in 15 minuts." });
...
router.post('/signup', async (req, res) => {
    ...
    await db.run(`INSERT INTO users (email, name, password) VALUES (?, ?, ?)`, [email, name, hashedPassword]);
...
router.post('/reset-password', async (req, res) => {
    ...
    await db.run(`UPDATE users SET password = ?, reset_token = NULL, reset_token_expires = NULL WHERE email = ?`, [hashedPassword, user.email]);
